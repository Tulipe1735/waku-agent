...
def test_append_search_human_edit_and_diff(tmp_path):
    ...
    assert entry["id"] == "F-001" and entry["author"] == "user"
    ...
    edited["body"] = "Human correction 14ms"
    ...
    assert "Human correction" in store.resume_context("research", query="correction")
...
def test_recovery_is_bounded_and_prioritizes_open_work(tmp_path):
    ...
    store.append(
        "task", {"kind": "question", "title": "Unresolved", "body": "Need owner", "status": "open"}
    )
    store.checkpoint("task")
    context = store.resume_context("task", max_tokens=500)
    assert "Need owner" in context
    assert len(context.encode()) <= 2000
...
def test_custom_storage_and_current_phase(tmp_path):
    ...
    assert store.append("task", finding())["phase"] == "review"
...
def test_other_process_lock_prevents_overwriting(tmp_path):
    ...
    with pytest.raises(FileExistsError):
        store.append("task", {"kind": "finding", "title": "race", "body": "race"})
