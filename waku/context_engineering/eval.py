...
VARIANTS = {
    ...
    "optimized": (True, True),
    "notebook-only": (True, False),
    "subagent-only": (False, True),
    "all-on": (True, True),
}
...
def deterministic_checks(case, artifact):
    ...
    if case.get("subagents") and gold.get("conflicts"):
        aggregation = artifact.get("aggregation") or {}
        conflicts = aggregation.get("metadata", {}).get("conflicts", [])
    ...
    for hypothesis in gold.get("hypotheses", []):
        ...
        state = continuation.get("metadata", {}) if isinstance(continuation, dict) else {}
