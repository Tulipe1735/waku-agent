...
def capture_case(case, variant, *, client=None, setup=None):
    ...
    flags = dict(
        zip(
            ("context_notebook", "context_subagents"),
            VARIANTS[variant],
            strict=True,
        )
    )
        ...
        try:
            ...
            if runtime is not None and flags["context_notebook"]:
                ...
                for entry in case.get("notebook", {}).get("entries", []):
                    runtime.notebook.append(
                        task_id,
                        {
                            "kind": entry["kind"],
                            "content": entry.get("body", ""),
                            "source_refs": [ref["ref"] for ref in entry.get("source_refs", [])],
                            "metadata": {
                                key: value
                                for key, value in entry.items()
                                if key not in {"kind", "body", "source_refs"}
                            },
                        },
                    )
            ...
            if runtime is not None and flags["context_notebook"]:
                ...
                continuation = notebook_checkpoint.get("continuation") if notebook_checkpoint else None
            ...
            artifacts = {}
            for root in (app.settings.home / "notebooks",):
                if root.exists():
            ...
            aggregation = (
                runtime.last_aggregation.to_dict() if runtime and runtime.last_aggregation else None
            )
            workers = aggregation["metadata"].get("results", []) if aggregation else []
            ...
            return {
                ...
                "subagent_success_rate": sum(
                    w["metadata"]["status"] == "success" for w in workers
                )
                / len(workers)
                if workers
                else None,
                "subagent_failure_rate": sum(
                    w["metadata"]["status"] != "success" for w in workers
                )
                / len(workers)
                if workers
